<div class="sidebar sidebar_margin">
	<p class="sidebar-sale sidebar-sale_view sidebar-sale_padding">
		<span class="sidebar-sale__span1">СКИДКИ</span><br>
		на цветы в коробках<br>
		<span class="sidebar-sale__span2">до 50%</span>
	</p>
	<div class="sidebar-mobile sidebar-mobile_view">
		<span>Дополнительные услуги</span>
	</div>
	<div class="sidebar-all">
		<div class="sidebar-box sidebar-box_view">
			<ul class="sidebar-menu">
				<li class="sidebar-menu__item sidebar-menu__item_first"><a href="#" class="sidebar-menu__link">Индивидуальный заказ</a></li>
				<li class="sidebar-menu__item sidebar-menu__item_first"><a href="#" class="sidebar-menu__link">Создать свой букет</a></li>
				<li class="sidebar-menu__item sidebar-menu__item_first"><a href="#" class="sidebar-menu__link">Букет сезона</a></li>
			</ul>
			<span>СВАДЕБНАЯ ФЛОРИСТИКА</span>
			<ul class="sidebar-menu sidebar-menu_margin">
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Букет невесты</a></li>
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Свадебное оформление</a></li>
			</ul>
		
			<span class="alone">ФЛОРИСТИЧЕСКОЕ ОФОРМЛЕНИЕ</span>
			<ul class="sidebar-menu">
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Праздники</a></li>
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Мероприятия</a></li>
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Интерьерные композиции</a></li>
			</ul>
		
			<span>ТРАУРНАЯ ФЛОРИСТИКА</span>
			<ul class="sidebar-menu">
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Траурные букеты</a></li>
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Венки из живых цветов</a></li>
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Оформление</a></li>
			</ul>
		
			<span>КОМНАТНЫЕ РАСТЕНИЯ</span>
			<ul class="sidebar-menu">
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Озеленение помещений</a></li>
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Уход</a></li>
				<li class="sidebar-menu__item"><a href="#" class="sidebar-menu__link">Композиции из комнатных<br>&nbsp;&nbsp;&nbsp;растений</a></li>
			</ul>
		</div>
		<div class="sidebar-testimonials sidebar-testimonials_view sidebar-testimonials_padding">
			<span class="sidebar-testimonials__span1">Отзывы</span>
			<div class="examples__slider">
				<div class="item">
					<p class="sidebar-testimonials__p">
						<span class="sidebar-testimonials__span2">Татьяна Иванова</span>
						<span class="sidebar-testimonials__span3">21.02.2017</span><br>
						Рыбным текстом называется текст,  служащий для <br>
						временного наполнения макета в публикациях или <br>
						производстве веб-сайтов, пока финальный текст еще не создан. Рыбный текст также известен как текст-заполнитель или же текст-
					</p>
				</div>
				<div class="item">
					<p class="sidebar-testimonials__p">
						<span class="sidebar-testimonials__span2">Татьяна Иванова</span>
						<span class="sidebar-testimonials__span3">21.02.2017</span><br>
						Рыбным текстом называется текст,  служащий для <br>
						временного наполнения макета в публикациях или <br>
						производстве веб-сайтов, пока финальный текст еще не создан. Рыбный текст также известен как текст-заполнитель или же текст-
					</p>
				</div>
				<div class="item">
					<p class="sidebar-testimonials__p">
						<span class="sidebar-testimonials__span2">Татьяна Иванова</span>
						<span class="sidebar-testimonials__span3">21.02.2017</span><br>
						Рыбным текстом называется текст,  служащий для <br>
						временного наполнения макета в публикациях или <br>
						производстве веб-сайтов, пока финальный текст еще не создан. Рыбный текст также известен как текст-заполнитель или же текст-
					</p>
				</div>
			</div>
			<a href="#" class="sidebar-testimonials__link">Все отзывы</a>
		</div>
		<div class="sidebar-work sidebar-work_border sidebar-work_margin">
			<span class="sidebar-work__title">Наши работы</span><br>
			<div class="sidebar-work__content">
				<a href="#" class="sidebar-work__link"><span class="sidebar-work__title2">Название нашей работы</span></a>
			</div>
		</div>
		<div class="sidebar-about sidebar-about_border">
			<p class="sidebar-about__content">
				<span>О нас</span><br>
				Рыбным текстом называется текст, служащий для временного 
				наполнения макета в публикациях или производстве веб-сайтов, пока 
				финальный текст еще не создан. Рыбный текст также известен как текст-заполнитель или же <br>
				<a href="#">Читать далее</a>
			</p>
		</div>
	</div>
</div>
